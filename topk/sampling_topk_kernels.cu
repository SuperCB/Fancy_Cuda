...
void invokeCurandBatchInitialize(curandState_t *states, const size_t batch_size,
                                 const unsigned long long *random_seeds,
                                 cudaStream_t stream) {
  dim3 block(256);
  dim3 grid((int)(ceil(batch_size * 1.0 / 256)));
  curandBatchInitialize<<<grid, block, 0, stream>>>(states, batch_size,
                                                    random_seeds);
}
...
#pragma unroll
    ...
    if (tid == 0) {
      ...
      s_val2[ite] = total.u;
      if (temp_top_k) {
        // printf("res_index %d,value is %f\n", res_index + ite, total.u);
        temp_top_k_res[res_index + ite] = total.u;
        printf("temp_top_k_res is %f", temp_top_k_res[res_index + ite]);
      }
      s_sum += total.u;
    }
    __syncthreads();
...
void top_k(const torch::Tensor src, const std::vector<int> &top_ks) {
  ...
  float *dst;
  cudaMalloc(&dst, sizeof(float) * batch_size);

  const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  top_k_cuda(src_ptr, dst, top_ks.data(), batch_size, len, stream, true,
             temp_top_k_res);
